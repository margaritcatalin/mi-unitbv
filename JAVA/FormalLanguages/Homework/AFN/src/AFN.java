...
public class AFN implements Serializable {
	...
	public AFN(String stareInitiala, ArrayList stariFinale, ArrayList alfabet, ArrayList arce) {
		super();
	...
	public String Verificare(ArrayList<Arc> arce, char lit, String stareInit) {
		...
		for (int i = 0; i < arce.size(); i++) {
			String[] tranz = arce.get(i).getSimbolArc().split(",");
			for (int j = 0; j < arce.get(i).getSimbolArc().split(",").length; j++)
				if (arce.get(i).getSt().equals(stareInit) && tranz[j].equals(Character.toString(lit))) {
